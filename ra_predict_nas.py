...
n_inputs = 69
n_outputs = 1
n_hidden_units_1 = 128
n_hidden_units_2 = 256
n_hidden_units_3 = 512
n_hidden_units_4 = 256
n_hidden_units_5 = 64
n_hidden_units_6 = 8
...
def add_layer(inputs, in_size, out_size, activation_function):
    ...
    Weights = tf.Variable(tf.random_uniform([in_size, out_size], minval=0.01, maxval=0.03))
    biases = tf.Variable(tf.zeros([1, out_size]) + 0.05)
...
x_data1 = x_data[69:101, :]
x_data2 = x_data[169:, :]
y_data1 = y_data[69:101]
y_data2 = y_data[169:]
x_data = np.vstack([x_data1, x_data2])
y_data = np.vstack([y_data1, y_data2])
scalerx = MinMaxScaler(feature_range=(0, 1))
tmp_data = scalerx.fit_transform(x_data[:, 32:37].reshape(-1, 1))
x_data[:, 32:37] = tmp_data.reshape(-1, 5)
...
with tf.Session() as sess:
    ...
    while i < epochs+1:
        step = 0
        while step < len(y_data[:40, :1])/batch_size+1:
        ...
        if i % 10 == 0:
            ...
            plt.savefig('picture/ra_markov_nas/' + str(i))
            plt.clf()
        i += 1
    save_path = saver.save(sess, "model/ra_markov_nas/model_"+now+'_e'+str(epochs)+'_b'+str(batch_size)+".ckpt")
    ...
    with open('result/ra_markov_nas/result_'+now+'_e'+str(epochs)+'_b'+str(batch_size)+'.csv', 'w', newline='') as csvfile:
    ...
    plt.savefig('picture/ra_markov_nas/'+now)
    plt.show()
    plt.clf()
